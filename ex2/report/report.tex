...
If we denote by $f^{i}$ the marginal distribution of the feature space which retains all features except the $i$th feature $x_i$, the PDP for $x_i$ feature can be expressed as
\begin{equation}
\text{PDP}_i(x) = \mathbb E_{\boldsymbol X}\left(Predict(X_1,\ldots,X_{i-1},x,X_{i+1},\ldots X_n)\right) % \int _{\mathbb R^n} Predict(x_1,\ldots,x_{i-1},x,x_{i+1},\ldots x_n) f(\boldsymbol x) d\boldsymbol x .
\end{equation}
Empirically, we can approximate the distribution of the feature space by the observed samples. Hence, at a given point $x$ the PDP can be found by setting the corresponding values of all samples in the dataset to $x$ and averaging over the predictions of the resulting modified dataset.
...
For a feature $X_i$ the ALE plot ALE$_i(x)$ can be defined differentially as



\begin{equation}
\frac{d}{dx} \text{ALE}_i (x) = \mathbb E_{\boldsymbol X | X_i}\left(\frac{d}{d_x} Predict(X_1,\ldots,X_{i-1},x,X_{i+1},\ldots X_n) | X_i=x\right)
\end{equation}
...
Another possibility for explaining the models' outcomes is training an interpretable surrogate model to the predictions that the original non-interpretable model makes. We trained a logistic regression classifier to the predictions our models made. Table~\ref{tab:logreg_results} shows the detection performance for logistic regression when training to labels our models predicted, and to the original lables, respectively. As expected, detection performance is considerably worse than for the more sophisticated models used in Section~\ref{sec:ml_approaches}. The fact that results for random forests are very similar to original data is due to the good detection performance of random forests. Hence, there is hardly any difference when training with the real labels and with the prediction of the random forest.
 ...
 Table~\ref{tab:logreg_coeff} displays the coefficients for 
\subsection{Interpretation}
