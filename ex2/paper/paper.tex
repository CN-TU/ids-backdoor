...
\caption{\gls{pdp} and \gls{ale} plots of the \gls{mlp} for the CIC-IDS-2017 classifier.} % The top plot shows the full range of TTL values while for the lower plot only \gls{ttl} values from 0 to 5 are displayed.}
\label{fig:pdp_ttl}
\end{figure}
...
\caption{Pruning process an \gls{rf} for \unsw{}, considering only harmless leaves and depth. Dash length proportional to validtion set size (from 0.01 until 1).}
