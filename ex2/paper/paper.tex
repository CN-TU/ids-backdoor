...
Finally, we also evaluate techniques proposed for \glspl{cnn} for removing backdoors from image classifiers, which, however, surprisingly turn out to be ineffective for our \gls{mlp} classifiers. 
...
For intrusion detection, decision tree based models or random forests of decision trees  usually achieve higher accuracy than \glspl{mlp} \cite{meghdouri_analysis_2018} and moreover they are generally faster to train and possibly more interpretable. For \glspl{ids} we thus consider the removal of backdoors from decision tree based models to be a more pressing issue than from \glspl{mlp}. 
