...
We discuss in this section different experimental results and observations. Overall, EagerNet shows promising performance comparable to \glspl{fcnn} with the ability to save resources. All experiments are reproducible and the code is available in our repository\footnote{\url{github.com/CN-TU/ids-backdoor/tree/eager}}.
...
\center
\begin{tabular}{cccc}
\subfloat[CIC-IDS17 (3 layers $\times$ 128 neurons).]{\includegraphics[width=0.78\columnwidth]{figures/conf-acc-17-3x128.pdf}} &
\subfloat[CIC-IDS17 (12 layers $\times$ 64 neurons).]{\includegraphics[width=0.78\columnwidth]{figures/conf-acc-17-12x64.pdf}}\\
...
In \autoref{tab:comparability}, we show the F1 score obtained at the last layer of three different architectures: Simple \gls{fcnn}, \gls{fcnn} with logistic output neuron per layer and EagerNet. \todo{Fares: continuation}
...
\center
\begin{tabular}{cccc}
\subfloat[CICIDS2017 (12 layers $\times$ 64 neurons).]{\includegraphics[width=0.55\columnwidth]{figures/multiclass17.pdf}} &
...
\todo{Max: Don't understand. Fares: Now added}
In order to understand how EagerNet determines which layer the best to stop evaluation, we use the multiclass variant to conduct an explanatory analysis. In \autoref{fig:multiclass}, we show the accuracy of the prediction obtained by different networks per layer and attack family. We observe two phenomena:
\begin{itemize}
\item  On average, the accuracy of predictions increases the deeper we get into the network. It was somehow expected, as more layers allow for more abstraction of the input space and thus better separation. However, only a few first layers are needed to obtain maximum accuracy for each attack family. This implies that the EagerNet architecture significantly decreases resources when used for intrusion detection applications.
\item \todo{Fares: recheck this} Some attacks families show extremely poor accuracy. This is due to two reasons: (1) too few samples from a category are mostly ignored by the network to reduce over-fitting and (2) If a specific layer focuses on learning such patterns, the weights are afterwards changed significantly to learn other majority classes which erases this ``fragile'' knowledge.
\end{itemize}
...
Training of weights on a given layer that are related to the output neuron can also be achieved by considering only the loss of that particular layer/output which just turns out to be a pure logistic regression between the $p^{th}$ layer and the $(p+1)^{th}$ layer output neuron. Nevertheless, due to the difficulty of the problem, constructing a linear combination of logits with adjustable weights provides limited transformations, and therefore inaccurate predictions, because the loss never completely converges. Back-propagating all losses at once is thus important and helps to update the weights of the network at the same time as adjusting the weights of the output neurons, thereby allowing the network to make accurate predictions.
